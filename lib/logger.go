/**
 *
 * Logger组件来自于框架，https://github.com/aceld//tree/master/zlog
 */
package lib
...
const (
	...
	BitLongFile                                            // 完整文件名称 /home/go/src/dir/log.go
	BitShortFile                                           // 最后文件名   log.go
		...
		if log.flag&(BitShortFile|BitLongFile) != 0 {
			...
			if log.flag&BitShortFile != 0 {
				...
				for i := len(file) - 1; i > 0; i-- {
					if file[i] == '/' {
						//找到最后一个'/'之后的文件名称  如:/home/go/src/log.go 得到 "log.go"
						short = file[i+1:]
						break
					}
				}
